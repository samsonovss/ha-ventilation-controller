...
Один и тот же физический вентилятор, выключатель или группу можно выбрать в нескольких экземплярах Ventilation Controller. Запросы объединяются по entity ID: вытяжка включается, если хотя бы одна комната просит усилить тягу, и выключается только когда ни одна комната больше её не просит. Если сущность выбрана только в одном экземпляре, поведение остаётся как у обычной локальной вытяжки.
...
- `Вытяжка`: `Выключено`, `Авто`, появляется только если выбрана вытяжка `fan`, `switch` или `group`
...
- Один и тот же `fan`/`switch`/`group` можно использовать в нескольких комнатах; запросы координируются, чтобы одна комната не выключила вытяжку, пока она нужна другой.
